...
import { useXboxGames } from "./Components/Custom/useXboxGames";
...
const Xbox = () => {
  ...
  return (
    <div className="pagina-principal">
      <Header
        onSearch={setSearch}
        onSearchEnter={(texto) => fetchXboxGames(texto)}
      />
      ...
      <main className="contenedor-games">
        {loadingXbox ? (
          <p className="cargando">
            {estaBuscando
              ? "Buscando juegos de Xbox..."
              : "Cargando videojuegos de Xbox..."}
          </p>
        ) : xboxGames.length > 0 ? (
          xboxGames.map((game) => {
          ...
          <p className="no-resultados">
            {estaBuscando
              ? `No se encontraron juegos de Xbox para "${search}".`
              : "No hay videojuegos de Xbox disponibles para este filtro."}
          </p>
        )}
      </main>
